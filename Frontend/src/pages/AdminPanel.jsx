...
function AdminPanel() {
  ...
  const cargarAuxiliares = async () => {
    try {
      const resMateriales = await axios.get('http://localhost:3000/api/productos/aux/materiales');
      const resTipos = await axios.get('http://localhost:3000/api/productos/aux/tipos');
  ...
  const manejarCambioTipoReporte = async (tipo) => {
    ...
    if (tipo === 'inventario') {
      ...
      try {
        ...
        const respuesta = await axios.get(`http://localhost:3000/api/ventas?fecha=${hoy}`);
  ...
  const procesarFiltroFechasVentas = async () => {
    ...
    try {
      const respuesta = await axios.get(`http://localhost:3000/api/ventas?desde=${fechaInicio}&hasta=${fechaFin}`);
      setDatosReporte(respuesta.data);
    } catch (error) {
  ...
  const guardarProducto = async (e) => {
    ...
    try {
      ...
      if (editandoId) {
        await axios.put(`http://localhost:3000/api/productos/${editandoId}`, datosAEnviar);
        alert('¡Joya actualizada con éxito!');
      } else {
        await axios.post('http://localhost:3000/api/productos', datosAEnviar);
  ...
  const manejarRebajoInventario = async (e) => {
    ...
    try {
      const respuesta = await axios.post('http://localhost:3000/api/productos/rebajar-stock-local', {
