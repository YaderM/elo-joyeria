...
export default function ReporteManager({ 
  ...
  // LOGICA FILTRO ESTADO (Añadido sin modificar tu estructura original)
          ...
          <tbody>
            {datosFiltrados.map((item, i) => (
              <tr key={i} style={{ borderBottom: '1px solid #eee' }}>
                ...
                <td style={estiloCeldaTd}>{esVentas ? (item.detalle_productos ? JSON.parse(item.detalle_productos).map(p => p.nombre).join(', ') : '-') : (item.nombre || 'Sin nombre')}</td>
